...
这是一个本地运行的 EVE 多角色工具，包含技能点查看、市场行情、农场财务、军团钱包总计和配装规划。

## 快速启动

1. 安装 Python 3.12 或更高版本，安装时勾选 `Add Python to PATH`。
2. 双击 `启动.bat`。
3. 首次启动会自动创建 `.env`，并提示填写你自己的 EVE 开发者应用凭据。
...
6. 将自己的 Client ID 和 Secret 填入 `.env`，再次双击 `启动.bat`。
...
物品索引已经附带，无需重新下载约 170MB 数据。首次运行会在本机创建虚拟环境并安装依赖。

## 隐私说明

- 分享包不包含原作者的 `.env`、角色令牌、角色数据库、钱包、财务或配装数据。
- 角色令牌只会保存在每位使用者自己的 `data/eve_esi.db`。
...
- 登录失败：检查 Client ID、Secret 和 Callback URL 是否完全一致。
